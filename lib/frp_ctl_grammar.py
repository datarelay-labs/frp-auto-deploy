...
def match(tokens, role, names=None, clients=None, groups=None):
    ...
    if verb in ("set", "unset", "create", "revoke", "purge", "release", "restore", "remove") and not server and verb != "set":
...
def _canonical_completion(tokens, trailing, role, names, services, local_services, groups=None):
    ...
    verb = filled[0]
    if verb == "help":
        topics = ["show", "set", "unset", "create", "update", "revoke", "purge", "release", "add", "remove", "legacy"]
        if len(filled) == 1:
