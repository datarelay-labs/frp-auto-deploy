...
grep -q 'PROJECT_VERSION=1.8.0' "$ROOT/VERSION" || fail "VERSION file"
...
grep -q 'PROJECT_VERSION=1.8.0' "$ENROLLED/etc/frp-auto-deploy/version" || fail "version migrated"
...
grep -q '1.1.0 -> 1.8.0' "$WORKDIR/enrolled.out" || fail "version transition"
...
grep -q 'Project version : 1.8.0' "$WORKDIR/enrolled-status.out" || fail "status after upgrade"
...
grep -q 'PROJECT_VERSION=1.8.0' "$LEGACY/etc/frp-auto-deploy/version" || fail "legacy version migration"
grep -q 'legacy / unknown -> 1.8.0' "$WORKDIR/legacy.out" || fail "legacy version transition"
...
grep -q 'Project version : 1.8.0' "$WORKDIR/legacy-after.out" || fail "legacy status version after"
