...
grep -q 'PROJECT_VERSION=1.4.0' "$TREE/etc/frp-auto-deploy/version" || fail "client project version"
...
grep -q 'Project version : 1.4.0' "$WORKDIR/status.out" || fail "status project version"
