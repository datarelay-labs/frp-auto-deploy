...
grep -q "Project version : ${PROJECT_VERSION}" "$WORKDIR/enrolled-status.out" || fail "status after upgrade"
...
grep -q "Project version : ${PROJECT_VERSION}" "$WORKDIR/legacy-after.out" || fail "legacy status version after"
