...
[[ ${#FRP_MATRIX_IMAGES[@]} -eq 6 ]] || fail "expected 6 default images"
[[ "${FRP_MATRIX_IMAGES[0]}" == ubuntu:22.04 ]] || fail "first image"
[[ "${FRP_MATRIX_IMAGES[5]}" == amazonlinux:2 ]] || fail "amazonlinux:2 required"
...
grep -q 'TOTAL=6' "$WORKDIR/all-pass.out" || fail "all-pass total"
grep -q 'PASSED=6' "$WORKDIR/all-pass.out" || fail "all-pass passed"
...
grep -q 'PASSED=5' "$WORKDIR/one-fail.out" || fail "continue-after-failure passed count"
